...
def get_batches(api_key,batch_no):
...
def get_batch_data(batch_id,batch_no):
    ...
    if batch_response.status_code == 200:
        ...
        print(f'Error retrieving batch {batch_no}:', batch_response.text)



batchno = 40
batches = get_batches(base64_authkey, batchno)

if len(batches) > 0:
    batch = batches[0]  # Retrieve the first (and only) batch from the response
    batch_id = batch['_id']
    print(batch_id)
    sql_insert = get_batch_data(batch_id,batchno)
else:
    print('Cant find a batch with batch number ',batchno)
